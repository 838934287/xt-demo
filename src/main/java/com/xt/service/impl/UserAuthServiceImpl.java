package com.xt.service.impl;

import com.xt.dao.UserAuthDao;
import com.xt.entity.UserAuth;
import com.xt.service.UserAuthService;
import org.springframework.stereotype.Service;

import javax.annotation.Resource;
import java.util.List;

/**
 * (UserAuth)表服务实现类
 *
 * @author makejava
 * @since 2020-03-28 19:56:30
 */
@Service("userAuthService")
public class UserAuthServiceImpl implements UserAuthService {
    @Resource
    private UserAuthDao userAuthDao;

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    @Override
    public UserAuth queryById(Integer id) {
        return this.userAuthDao.queryById(id);
    }

    /**
     * 查询多条数据
     *
     * @param offset 查询起始位置
     * @param limit 查询条数
     * @return 对象列表
     */
    @Override
    public List<UserAuth> queryAllByLimit(int offset, int limit) {
        return this.userAuthDao.queryAllByLimit(offset, limit);
    }

    /**
     * 新增数据
     *
     * @param userAuth 实例对象
     * @return 实例对象
     */
    @Override
    public UserAuth insert(UserAuth userAuth) {
        this.userAuthDao.insert(userAuth);
        return userAuth;
    }

    /**
     * 修改数据
     *
     * @param userAuth 实例对象
     * @return 实例对象
     */
    @Override
    public UserAuth update(UserAuth userAuth) {
        this.userAuthDao.update(userAuth);
        return this.queryById(userAuth.getId());
    }

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 是否成功
     */
    @Override
    public boolean deleteById(Integer id) {
        return this.userAuthDao.deleteById(id) > 0;
    }

    @Override
    public List<UserAuth> queryByConditon(UserAuth userAuth){
        return this.userAuthDao.queryByConditon(userAuth);
    }
}